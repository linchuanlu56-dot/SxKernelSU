...
SxKernelSU 官方支持 GKI 2.0 的设备（内核版本5.10以上）；旧内核也是兼容的（最低4.14+），不过需要自己编译内核。

WSA, ChromeOS 和运行在容器上的 Android 也可以与 SxKernelSU 一起工作。

目前支持 `arm64-v8a` 和 `x86_64` 架构。

> [!CAUTION]
> 最近的内核版本引入了一项破坏性更改，导致 SxKernelSU 在 `x86_64` 上运行失败，甚至可能引发内核恐慌 (kernel panic)！请查看网站获取更多信息！
...
要将 SxKernelSU 翻译成您的语言，或完善现有的翻译，请使用 [Weblate](https://hosted.weblate.org/engage/kernelsu/)。现已不再接受有关管理器翻译的PR，因为这会与Weblate冲突。
...
有关报告 SxKernelSU 安全漏洞的信息，请参阅 [SECURITY.md](/SECURITY.md)。
...
- [kernel-assisted-superuser](https://git.zx2c4.com/kernel-assisted-superuser/about/)：SxKernelSU 的灵感。
