...
SxKernelSU использует архитектуру [метамодулей](metamodule.md) для монтирования директории `system`. **Только если вашему модулю нужно модифицировать файлы `/system`** (через директорию `system`), вам необходимо установить метамодуль (например, [meta-overlayfs](https://github.com/linchuanlu56-dot/SxKernelSU/releases)). Другие функции модулей, такие как скрипты, правила sepolicy и system.prop, работают без метамодуля.
:::
...
1. Установите переменной окружения `ASH_STANDALONE` значение `1`<br>Пример: `ASH_STANDALONE=1 /data/adb/sks/bin/busybox sh <script>`
2. Переключитесь с помощью параметров командной строки:<br>`/data/adb/sks/bin/busybox sh -o standalone <script>`
...
## Модули SxKernelSU {#sxkernelsu-modules}
...
- `SKS_VER` (string): строка версии текущего установленного SxKernelSU (например, `v0.4.0`)
- `SKS_VER_CODE` (int): код версии текущего установленного SxKernelSU в пользовательском пространстве (например, `10672`)
- `SKS_KERNEL_VER_CODE` (int): код версии текущей установленной SxKernelSU в пространстве ядра (например, `10672`)
...
Late-load запускается командой `sksud late-load`. Эта команда:

1. Определяет текущую версию KMI и загружает соответствующий `sxkernelsu.ko` из встроенных ресурсов.
...
| Хуки kprobe sksud (execve/read/fstat/input) | Да | Пропущены |
...
| Переменная окружения `SKS_LATE_LOAD` | Не установлена | Установлена в `1` |
...
```txt
sksud late-load:
  1. Загрузить sxkernelsu.ko (если ещё не загружен)
...
Модули могут определить режим late-load, проверив переменную окружения `SKS_LATE_LOAD`:
