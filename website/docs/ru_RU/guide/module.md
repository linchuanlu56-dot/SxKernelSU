...
SxKernelSU предоставляет механизм модулей, позволяющий добиться эффекта модификации системного каталога при сохранении целостности системного раздела. Этот механизм принято называть "бессистемным".

Модульный механизм SxKernelSU практически аналогичен механизму Magisk. Если вы знакомы с разработкой модулей Magisk, то разработка модулей SxKernelSU очень похожа. Представление модулей ниже можно пропустить, достаточно прочитать [различия-с-magisk] (difference-with-magisk.md).

::: warning МЕТАМОДУЛЬ ТРЕБУЕТСЯ ТОЛЬКО ДЛЯ МОДИФИКАЦИИ СИСТЕМНЫХ ФАЙЛОВ
SxKernelSU использует архитектуру [метамодулей](metamodule.md) для монтирования директории `system`. **Только если вашему модулю нужно модифицировать файлы `/system`** (через директорию `system`), вам необходимо установить метамодуль (например, [meta-overlayfs](https://github.com/tiann/SxKernelSU/releases)). Другие функции модулей, такие как скрипты, правила sepolicy и system.prop, работают без метамодуля.
:::

## WebUI

SxKernelSU modules support displaying interfaces and interacting with users. See the [WebUI documentation](module-webui.md) for additional details.

## Конфигурация модулей

SxKernelSU предоставляет встроенную систему конфигурации, которая позволяет модулям хранить постоянные или временные настройки в формате ключ-значение. Подробности смотрите в [документации по конфигурации модулей](module-config.md).

## Busybox

В комплект поставки SxKernelSU входит полнофункциональный бинарный файл BusyBox (включая полную поддержку SELinux). Исполняемый файл находится по адресу `/data/adb/ksu/bin/busybox`. BusyBox от SxKernelSU поддерживает переключаемый во время работы "ASH Standalone Shell Mode". Этот автономный режим означает, что при запуске в оболочке `ash` BusyBox каждая команда будет напрямую использовать апплет внутри BusyBox, независимо от того, что задано в качестве `PATH`. Например, такие команды, как `ls`, `rm`, `chmod` будут **НЕ** использовать то, что находится в `PATH` (в случае Android по умолчанию это будут `/system/bin/ls`, `/system/bin/rm` и `/system/bin/chmod` соответственно), а вместо этого будут напрямую вызывать внутренние апплеты BusyBox. Это гарантирует, что скрипты всегда будут выполняться в предсказуемом окружении и всегда будут иметь полный набор команд, независимо от того, на какой версии Android они выполняются. Чтобы заставить команду _не_ использовать BusyBox, необходимо вызвать исполняемый файл с полными путями.

Каждый сценарий оболочки, запущенный в контексте SxKernelSU, будет выполняться в оболочке BusyBox `ash` с включенным автономным режимом. Для сторонних разработчиков это касается всех загрузочных скриптов и скриптов установки модулей.

Для тех, кто хочет использовать эту возможность "Автономного режима" вне SxKernelSU, есть два способа включить ее:
...
Чтобы убедиться, что все последующие запуски оболочки `sh` также выполняются в автономном режиме, предпочтительным методом является вариант 1 (и это то, что SxKernelSU и менеджер SxKernelSU используют внутри), поскольку переменные окружения наследуются вплоть до дочерних процессов.

::: tip отличие от Magisk

BusyBox в SxKernelSU теперь использует бинарный файл, скомпилированный непосредственно из проекта Magisk. **Поэтому вам не нужно беспокоиться о проблемах совместимости между скриптами BusyBox в Magisk и SxKernelSU, поскольку они абсолютно одинаковы!
:::

## Модули SxKernelSU {#kernelsu-modules}

Модуль SxKernelSU - это папка, размещенная в каталоге `/data/adb/modules` и имеющая следующую структуру:
...
│   │      *** Флаги состояния ***
│   │
│   ├── skip_mount          <--- Если он существует, то SxKernelSU НЕ будет монтировать вашу системную папку
...
|   ├── uninstall.sh        <--- Этот скрипт будет выполнен, когда SxKernelSU удалит ваш модуль
...
SxKernelSU не имеет встроенной поддержки Zygisk, поэтому в модуле нет содержимого, связанного с Zygisk. Однако для поддержки модулей Zygisk можно использовать [ZygiskNext](https://github.com/Dr-TSNG/ZygiskNext). В этом случае содержимое модуля Zygisk идентично содержимому, поддерживаемому Magisk.
:::

### module.prop

module.prop - это конфигурационный файл модуля. В SxKernelSU, если модуль не содержит этого файла, он не будет распознан как модуль. Формат этого файла следующий:
...
::: tip различия с Magisk
С помощью переменной окружения KSU можно определить, выполняется ли сценарий в SxKernelSU или в Magisk. Если скрипт выполняется в SxKernelSU, то это значение будет равно true.
:::
...
Вы также можете объявить в `customize.sh` переменную с именем `REMOVE`, содержащую список каталогов для выполнения операций удаления, и SxKernelSU автоматически выполнит команду `mknod <TARGET> c 0 0` в соответствующих каталогах модуля. Например:
...
Вы можете объявить в файле `customize.sh` переменную с именем `REPLACE`, содержащую список заменяемых каталогов, и SxKernelSU автоматически выполнит соответствующие операции в каталоге вашего модуля. Например:
...
SxKernelSU предоставляет механизм для внедрения кастомных директив Android Init RC в системный `init.rc`. Это позволяет модулям регистрировать кастомные службы Android, устанавливать триггеры свойств или выполнять другие действия на языке Init без изменения системного раздела.

Во время загрузки модуль ядра SxKernelSU перехватывает системные вызовы `read()` и `fstat()`. Когда процесс init Android читает `/system/etc/init/hw/init.rc`, SxKernelSU прозрачно добавляет кастомное содержимое RC в конец файла. Процесс init анализирует эти внедренные директивы точно так же, как исходное содержимое init.rc.
...
Инсталлятор модуля SxKernelSU - это модуль SxKernelSU, упакованный в zip-файл, который может быть прошит в APP-менеджере SxKernelSU. Простейший установщик модуля SxKernelSU - это просто модуль SxKernelSU, упакованный в zip-файл.
...
:::warning
Модуль SxKernelSU НЕ поддерживается для установки в пользовательское Recovery!!!
:::
...
Сценарий `customize.sh` запускается в оболочке BusyBox `ash` SxKernelSU с включенным "Автономным режимом". Доступны следующие переменные и функции:

#### Переменные

- `KSU` (bool): переменная, отмечающая, что скрипт выполняется в окружении SxKernelSU, причем значение этой переменной всегда будет true. Ее можно использовать для различения SxKernelSU и Magisk.
- `KSU_VER` (string): строка версии текущего установленного SxKernelSU (например, `v0.4.0`)
- `KSU_VER_CODE` (int): код версии текущего установленного SxKernelSU в пользовательском пространстве (например, `10672`)
- `KSU_KERNEL_VER_CODE` (int): код версии текущей установленной SxKernelSU в пространстве ядра (например, `10672`)
- `BOOTMODE` (bool): в SxKernelSU всегда должно быть `true`.
...
- `KSU_UAPI_VER` (int): версия UAPI пользовательского пространства SxKernelSU (ksud) (например, `2`). Эта версия увеличивается при критических изменениях в драйвере ядра, и модули могут использовать её для проверки совместимости.
- `KSU_RUNTIME_MODE` (string): текущий режим работы SxKernelSU. Возможные значения: `built-in` (режим GKI, встроен в ядро), `lkm` (загружен как модуль ядра при запуске) или `late-load` (загружен как модуль ядра после загрузки).
- `KSU_LATE_LOAD` (int?): если SxKernelSU загружен с задержкой после загрузки системы, значение этой переменной равно `1`; в противном случае переменная не устанавливается.

::: warning
В SxKernelSU MAGISK_VER_CODE всегда равен 25200, а MAGISK_VER всегда равен v25.2. Пожалуйста, не используйте эти две переменные для определения того, запущен ли он на SxKernelSU или нет.
:::
...
В SxKernelSU скрипты делятся на два типа в зависимости от режима их работы: режим post-fs-data и режим late_start service:
...
В SxKernelSU скрипты запуска делятся на два типа по месту их хранения: общие скрипты и скрипты модулей:
...
Все загрузочные скрипты будут выполняться в оболочке BusyBox `ash` от SxKernelSU с включенным "Автономным режимом".

## Режим late-load {#late-load-mode}

Помимо стандартного процесса загрузки, описанного выше, SxKernelSU поддерживает **режим late-load** для сценариев LKM (загружаемый модуль ядра). В этом режиме модуль ядра SxKernelSU загружается **после полной загрузки системы**, а не во время процесса init.
