...
Metamodul adalah fitur revolusioner di SxKernelSU yang mentransfer kemampuan sistem modul yang penting dari daemon inti ke modul yang dapat dipasang. Pergeseran arsitektur ini mempertahankan stabilitas dan keamanan SxKernelSU sambil melepaskan potensi inovasi yang lebih besar untuk ekosistem modul.

## Apa itu Metamodul?

Metamodul adalah jenis modul SxKernelSU khusus yang menyediakan fungsi infrastruktur inti untuk sistem modul. Tidak seperti modul biasa yang memodifikasi file sistem, metamodul mengontrol *bagaimana* modul biasa diinstal dan dipasang.

Metamodul adalah mekanisme ekstensi berbasis plugin yang memungkinkan kustomisasi lengkap infrastruktur manajemen modul SxKernelSU. Dengan mendelegasikan logika pemasangan dan instalasi ke metamodul, SxKernelSU menghindari menjadi titik deteksi yang rapuh sambil memungkinkan strategi implementasi yang beragam.
...
Solusi root tradisional memasukkan logika pemasangan ke dalam inti mereka, membuat mereka lebih mudah dideteksi dan lebih sulit untuk berkembang. Arsitektur metamodul SxKernelSU memecahkan masalah ini melalui pemisahan perhatian.

**Keunggulan strategis:**

- **Mengurangi permukaan deteksi**: SxKernelSU sendiri tidak melakukan pemasangan, mengurangi vektor deteksi
- **Stabilitas**: Daemon inti tetap stabil sementara implementasi pemasangan dapat berkembang
- **Inovasi**: Komunitas dapat mengembangkan strategi pemasangan alternatif tanpa mem-fork SxKernelSU
...
- **Ekstensibilitas**: Tambahkan fitur seperti dukungan modul kernel tanpa memodifikasi inti SxKernelSU
- **Modularitas**: Perbarui implementasi secara independen dari rilis SxKernelSU
- **Kustomisasi**: Buat solusi khusus untuk perangkat atau kasus penggunaan tertentu

::: warning PENTING
Tanpa metamodul yang diinstal, modul **TIDAK** akan dipasang. Instalasi SxKernelSU yang baru memerlukan pemasangan metamodul (seperti `meta-overlayfs`) agar modul berfungsi.
:::
...
2. Buka aplikasi SxKernelSU Manager
...
Anda dapat memeriksa metamodul mana yang saat ini aktif di halaman Modul aplikasi SxKernelSU Manager. Metamodul aktif akan ditampilkan di daftar modul Anda dengan penunjukan khususnya.
...
Hanya satu metamodul yang dapat diinstal pada satu waktu. Jika Anda mencoba menginstal metamodul kedua, SxKernelSU akan mencegah instalasi untuk menghindari konflik.
...
Jika Anda mengembangkan modul SxKernelSU biasa, Anda tidak perlu terlalu khawatir tentang metamodul. Modul Anda akan berfungsi selama pengguna memiliki metamodul yang kompatibel (seperti `meta-overlayfs`) yang diinstal.
...
::: tip
Jika Anda terbiasa dengan pengembangan modul Magisk, modul Anda akan berfungsi dengan cara yang sama di SxKernelSU ketika metamodul diinstal, karena menyediakan pemasangan kompatibel Magisk.
:::

## Untuk Pengembang Metamodul

Membuat metamodul memungkinkan Anda untuk menyesuaikan bagaimana SxKernelSU menangani instalasi modul, pemasangan, dan penghapusan instalasi.
...
- Semua variabel lingkungan SxKernelSU standar
...
Saat melakukan operasi mount, Anda **HARUS** mengatur nama sumber/perangkat ke `"KSU"`. Ini mengidentifikasi mount sebagai milik SxKernelSU.
...
Ini penting agar SxKernelSU mengidentifikasi dan mengelola mount-nya dengan benar.
:::
...
- **Variabel**: `MODPATH`, `TMPDIR`, `ZIPFILE`, `ARCH`, `API`, `IS64BIT`, `KSU`, `SKS_VER`, `SKS_VER_CODE`, `BOOTMODE`, dll.
- **Fungsi**:
...
Ketika metamodul diinstal, SxKernelSU membuat symlink:
...
1. **Uji instalasi** pada pengaturan SxKernelSU yang bersih
...
- [Perbedaan dengan Magisk](difference-with-magisk.md) - Membandingkan SxKernelSU dan Magisk
- [Cara Membangun](how-to-build.md) - Membangun SxKernelSU dari sumber
