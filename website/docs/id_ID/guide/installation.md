...
Unduh manajer SxKernelSU dari [GitHub Releases](https://github.com/linchuanlu56-dot/SxKernelSU/releases) dan instal ke perangkat Anda:

- Jika aplikasi menunjukkan `Unsupported`, itu berarti **Anda harus mengkompilasi kernel sendiri**, SxKernelSU tidak akan dan tidak pernah menyediakan file boot.img untuk Anda flash.
- Jika aplikasi menunjukkan `Not installed`, maka perangkat Anda secara resmi didukung oleh SxKernelSU.
...
Sejak versi [0.9.0](https://github.com/linchuanlu56-dot/SxKernelSU/releases/tag/v0.9.0), SxKernelSU mendukung dua mode berjalan pada perangkat GKI:

1. `GKI`: Ganti kernel asli perangkat dengan **Generic Kernel Image** (GKI) yang disediakan oleh SxKernelSU.
...
Dalam mode GKI, kernel asli perangkat akan diganti dengan image kernel generik yang disediakan oleh SxKernelSU. Keuntungan mode GKI adalah:
...
1. Tidak akan mengganti kernel asli perangkat. Jika Anda memiliki persyaratan khusus untuk kernel asli perangkat, atau Anda ingin menggunakan SxKernelSU sambil menggunakan kernel pihak ketiga, Anda dapat menggunakan mode LKM.
2. Lebih nyaman untuk upgrade dan OTA. Saat mengupgrade SxKernelSU, Anda dapat langsung menginstalnya di manajer tanpa flashing manual. Setelah OTA sistem, Anda dapat langsung menginstalnya ke slot kedua tanpa flashing manual.
...
Alat ini mendukung macOS, Linux, dan Windows. Anda dapat mengunduh versi yang sesuai dari [GitHub Release](https://github.com/linchuanlu56-dot/SxKernelSU/releases).
...
1. Instal dengan fastboot menggunakan boot.img yang disediakan oleh SxKernelSU.
...
## Instal dengan boot.img yang disediakan oleh SxKernelSU

Jika `boot.img` perangkat Anda menggunakan format kompresi yang umum digunakan, Anda dapat menggunakan image GKI yang disediakan oleh SxKernelSU untuk mem-flash-nya langsung. Ini tidak memerlukan TWRP atau self-patching image.

### Temukan boot.img yang tepat

SxKernelSU menyediakan boot.img generik untuk perangkat GKI, dan Anda harus mem-flash boot.img ke partisi boot perangkat.

Anda dapat mengunduh boot.img dari [GitHub Release](https://github.com/linchuanlu56-dot/SxKernelSU/releases). Harap dicatat bahwa Anda harus menggunakan versi boot.img yang benar. Jika Anda tidak tahu file mana yang harus diunduh, baca dengan cermat deskripsi [KMI](#kmi) dan [Tingkat patch keamanan](#security-patch-level) dalam dokumen ini.
...
Gunakan `adb` untuk menghubungkan perangkat Anda, lalu jalankan `adb reboot bootloader` untuk masuk ke mode fastboot, dan gunakan perintah ini untuk mem-flash SxKernelSU:
...
1. Perangkat Anda telah di-root. Misalnya, Anda telah menginstal SxKernelSU dan ingin mengupgrade ke versi terbaru atau Anda telah di-root melalui metode lain (seperti Magisk).
2. Jika perangkat Anda tidak di-root, tetapi perangkat mendukung metode boot sementara `fastboot boot boot.img`, Anda dapat menggunakan image GKI yang disediakan oleh SxKernelSU untuk boot sementara perangkat Anda, mendapatkan izin root sementara, lalu gunakan aplikasi Kernel Flash untuk mendapatkan hak root permanen.
...
2. Unduh file ZIP AnyKernel3 yang disediakan oleh SxKernelSU yang cocok dengan versi KMI perangkat Anda. Anda dapat merujuk ke [Instal dengan Recovery kustom](#install-with-custom-recovery).
3. Unpack paket AnyKernel3 dan dapatkan file `Image`, yang merupakan file kernel SxKernelSU.
...
Faktanya, semua metode instalasi ini hanya memiliki satu ide utama, yaitu **mengganti kernel asli dengan yang disediakan oleh SxKernelSU**, selama ini dapat dicapai, dapat diinstal. Berikut adalah metode lain yang mungkin:

1. Pertama, instal Magisk, dapatkan hak root melalui Magisk, lalu gunakan Kernel Flasher untuk mem-flash AnyKernel3 ZIP dari SxKernelSU.
2. Gunakan toolkit flashing apa pun di PC untuk mem-flash kernel yang disediakan oleh SxKernelSU.
...
Jika Anda ingin menggunakan modul yang memodifikasi file `/system`, Anda perlu menginstal **metamodule** setelah menginstal SxKernelSU. Modul yang hanya menggunakan skrip, sepolicy, atau system.prop bekerja tanpa metamodule.
:::
