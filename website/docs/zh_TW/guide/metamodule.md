...
元模組是 SxKernelSU 的一項革命性功能,它將關鍵的模組系統能力從核心守護程序轉移到可插拔模組中。這種架構轉變在保持 SxKernelSU 穩定性和安全性的同時,為模組生態系統釋放了更大的創新潛力。

## 什麼是元模組?

元模組是一種特殊類型的 SxKernelSU 模組,為模組系統提供核心基礎設施功能。與修改系統檔案的常規模組不同,元模組控制常規模組的*安裝和掛載方式*。

元模組是一種基於外掛的擴充機制,允許完全自訂 SxKernelSU 的模組管理基礎設施。透過將掛載和安裝邏輯委託給元模組,SxKernelSU 避免成為脆弱的檢測點,同時支援多樣化的實作策略。
...
傳統的 Root 解決方案將掛載邏輯內建在核心中,這使得它們更容易被檢測且難以演進。SxKernelSU 的元模組架構透過關注點分離解決了這些問題。

**策略優勢:**

- **減少檢測面**: SxKernelSU 本身不執行掛載,減少了檢測向量
- **穩定性**: 核心守護程序保持穩定,而掛載實作可以不斷演進
- **創新性**: 社群可以開發替代掛載策略,而無需分叉 SxKernelSU
...
- **可擴充性**: 新增核心模組支援等功能,無需修改核心 SxKernelSU
- **模組化**: 獨立於 SxKernelSU 版本更新實作
- **客製化**: 為特定裝置或用例建立專門的解決方案

::: warning 重要
如果沒有安裝元模組,模組將**不會**被掛載。新安裝的 SxKernelSU 需要安裝元模組(如 `meta-overlayfs`)才能使模組正常運作。
:::
...
您可以在 SxKernelSU Manager 應用程式的模組頁面中檢視目前活動的元模組。活動的元模組將顯示在模組清單中,並帶有特殊標識。
...
一次只能安裝一個元模組。如果您嘗試安裝第二個元模組,SxKernelSU 將阻止安裝以避免衝突。
...
如果您正在開發常規 SxKernelSU 模組,您不需要太擔心元模組。只要使用者安裝了相容的元模組(如 `meta-overlayfs`),您的模組就能正常運作。
...
::: tip
如果您熟悉 Magisk 模組開發,您的模組在安裝元模組後將在 SxKernelSU 中以相同方式運作,因為它提供了 Magisk 相容的掛載。
:::

## 對於元模組開發者

建立元模組允許您自訂 SxKernelSU 處理模組安裝、掛載和解除安裝的方式。
...
::: danger 關鍵要求
執行掛載操作時,**必須**將來源/裝置名稱設定為 `"KSU"`。這將掛載標識為屬於 SxKernelSU。
...
- **變數**: `MODPATH`、`TMPDIR`、`ZIPFILE`、`ARCH`、`API`、`IS64BIT`、`KSU`、`SKS_VER`、`SKS_VER_CODE`、`BOOTMODE` 等
- **函式**:
...
- [與 Magisk 的差異](difference-with-magisk.md) - 比較 SxKernelSU 和 Magisk
- [如何建置](how-to-build.md) - 從原始碼建置 SxKernelSU
