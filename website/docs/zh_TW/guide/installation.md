...
從 [GitHub Releases](https://github.com/linchuanlu56-dot/SxKernelSU/releases) 下載 SxKernelSU 管理器，然後安裝至裝置並開啟：

- 如果顯示「不支援」，則表示您的裝置不支援 SxKernelSU，您需要自行編譯核心才能繼續使用，SxKernelSU 官方也永遠不會提供一個您可以寫入的 Boot 映像。
- 如果顯示「未安裝」，那麼 SxKernelSU 支援您的裝置。
...
自 `0.9.0` 版本以後，在 GKI 裝置上，SxKernelSU 支援兩種運作模式：
...
GKI 模式會替換掉裝置原有的核心，使用 SxKernelSU 提供的通用核心鏡像。 GKI 模式的優點是：
...
1. 不會取代裝置原有的核心：如果你對裝置原有的核心有特殊需求，或是你希望在使用第三方核心的同時使用 SxKernelSU，可以使用 LKM 模式。
2. 升級和 OTA 較為方便：升級 SxKernelSU 時，可以直接在管理器內部安裝，無需再手動寫入；系統 OTA 後，可以直接安裝到第二個槽位，也無需再手動寫入。
...
取得官方韌體的方法有很多，如果你的裝置支援 `fastboot boot`，那麼我們最推薦以及最簡單的方法是使用 `fastboot boot` 臨時啟動 SxKernelSU 提供的 GKI 核心，並參考[使用管理器](#use-the-manager)安裝。
...
2. 直接安裝：如果你手機已經 root，你可以選擇這個選項，管理器會自動獲取你的裝置資訊，然後自動修補官方韌體，然後寫入。你可以考慮使用 `fastboot boot` SxKernelSU 的 GKI 核心來取得臨時 root 安裝管理器，然後再使用這個選項。**這種方式也是 SxKernelSU 升級最主要的方式**。
...
如果你不想使用管理器，你也可以使用命令列來安裝 LKM。SxKernelSU 提供的 `sksud` 可以幫助你快速修補官方韌體，然後寫入。

這個工具支援 macOS、Linux 和 Windows，你可以在 [GitHub Release](https://github.com/linchuanlu56-dot/SxKernelSU/releases) 下載對應的版本。

使用方法：`sksud boot-patch`。 你可以查看命令列的提示了解具體的使用方法。

```sh
husky:/ # sksud boot-patch -h
...
```
需要說明的幾個選項：
1. `--magiskboot` 選項可以指定 magiskboot 的路徑，如果不指定，sksud 會在環境變數中尋找。如果你不知道如何取得 magiskboot，可以參考[這裡](#patch-boot-image)。
...
1. 使用 SxKernelSU 提供的 boot.img 透過 Fastboot 安裝
2. 使用核心寫入程式 (例如 KernelFlasher) 安裝
3. 使用自訂 Recovery (例如 TWRP) 安裝
4. 手動修補 boot.img 並安裝

## 使用 SxKernelSU 提供的 boot.img 安裝 {#install-with-boot-img-provided-by-sxkernelsu}

如果你的裝置的 `boot.img` 使用常見的壓縮格式，你可以直接寫入 SxKernelSU 提供的 GKI 核心映像，這種方法無需 TWRP，也無需您的手機有 Root 權限；適用於您初次安裝 SxKernelSU。

### 找到合適的 boot.img {#find-proper-boot-img}

SxKernelSU 為 GKI 裝置提供了標準 boot.img，您需要將 boot.img 寫入至裝置的 Boot 分區。

您可以從 [GitHub Release](https://github.com/linchuanlu56-dot/SxKernelSU/releases) 下載 boot.img，請注意，您應該使用正確版本的 boot.img。如果你不知道你該下載哪個檔案，請詳細閱讀文檔中的 [KMI](#kmi) 與[安全性修補程式等級](#security-patch-level)。
...
使用 `adb` 連接您的裝置，然後執行 `adb reboot bootloader` 進入 fastboot 模式，然後使用此命令寫入 SxKernelSU：
...
先決條件：您的裝置必須已經 Root。例如您已經安裝了 Magisk 並取得 Root 存取權，或者您已經安裝了舊版本的 SxKernelSU 需升級到其他版本的 SxKernelSU；如果您的裝置並未 Root，請嘗試其他方法。
...
P.S. 這種方法在更新 SxKernelSU 時比較方便，無需電腦即可完成 (注意備份！)。

## 手動修補 boot.img {#patch-boot-image}

對於某些裝置來說，其 boot.img 格式並不是很常見，不屬於 `lz4`，`gz` 和未壓縮；最典型的就是 Pixel，它的 boot.img 格式是 `lz4_legacy` 壓縮，ramdisk 可能是 `gz` 也可能是 `lz4_legacy` 壓縮；此時如果您直接寫入 SxKernelSU 提供的 boot.img，手機可能無法開機。這時，您可以透過手動修補 boot.img 來完成。
...
2. 下載 SxKernelSU 提供的與您的裝置 KMI 一致的 AnyKernel3 Zip 檔 (可參閱[使用自訂 Recovery 安裝](#install-with-custom-recovery))。
3. 解壓縮 AnyKernel3 Zip 檔，取得其中的 `Image` 檔，此檔案為具有 SxKernelSU 的核心。
...
1. 在 SxKernelSU 的 [Release 頁面](https://github.com/linchuanlu56-dot/SxKernelSU/releases) 下載與您手機版本相符的以 AnyKernel3 開頭的 Zip 檔；例如，手機核心版本為 `android12-5.10.66`，那麼您應該下載 `AnyKernel3-android12-5.10.66_yyyy-MM.zip` 這個檔案 (其中 `yyyy` 為年份，`MM` 為月份)。
2. 重新開機手機至 TWRP。
...
其實所有這些安裝方法的主旨只有一個，那就是**將原廠核心取代為 SxKernelSU 提供的核心**。只要能實現這個目的，就可以安裝，比如以下是其他可行的方法：

1. 首先安裝 Magisk，透過 Magisk 取得 Root 權限後使用核心寫入程式寫入 SxKernelSU 的 AnyKernel Zip。
2. 使用某些 PC 上的寫入工具組寫入 SxKernelSU 提供的核心。
...
如果您想使用修改 `/system` 檔案的模組，您需要在安裝 SxKernelSU 後安裝 **metamodule**。僅使用腳本、sepolicy 或 system.prop 的模組無需 metamodule 即可運作。
:::
