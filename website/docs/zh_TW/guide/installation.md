...
從 [GitHub Releases](https://github.com/tiann/SxKernelSU/releases) 下載 SxKernelSU 管理器，然後安裝至裝置並開啟：
...
這個工具支援 macOS、Linux 和 Windows，你可以在 [GitHub Release](https://github.com/tiann/SxKernelSU/releases) 下載對應的版本。
...
您可以從 [GitHub Release](https://github.com/tiann/SxKernelSU/releases) 下載 boot.img，請注意，您應該使用正確版本的 boot.img。如果你不知道你該下載哪個檔案，請詳細閱讀文檔中的 [KMI](#kmi) 與[安全性修補程式等級](#security-patch-level)。
...
1. 在 SxKernelSU 的 [Release 頁面](https://github.com/tiann/SxKernelSU/releases) 下載與您手機版本相符的以 AnyKernel3 開頭的 Zip 檔；例如，手機核心版本為 `android12-5.10.66`，那麼您應該下載 `AnyKernel3-android12-5.10.66_yyyy-MM.zip` 這個檔案 (其中 `yyyy` 為年份，`MM` 為月份)。
2. 重新開機手機至 TWRP。
