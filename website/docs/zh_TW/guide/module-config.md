...
SxKernelSU 提供了一個內建的配置系統,允許模組儲存持久化或暫時的鍵值設定。配置以二進位格式儲存在 `/data/adb/sks/module_configs/<module_id>/`,具有以下特性:
...
所有模組腳本(`post-fs-data.sh`、`service.sh`、`boot-completed.sh` 等)執行時都會設定 `SKS_MODULE` 環境變數為模組 ID。您可以使用 `sksud module config` 命令來管理模組的配置:
...
- 在腳本中使用配置值之前驗證它們
- 使用 `sksud module config list` 命令偵錯配置問題
:::
...
```bash
# 覆蓋模組描述
sksud module config set override.description "在管理器中顯示的自訂描述"
```
...
模組可以使用 `manage.<feature>` 配置模式宣告它們管理的 SxKernelSU 功能。支援的功能對應於 SxKernelSU 內部的 `FeatureId` 列舉:
...
- SxKernelSU 管理器偵測哪些模組管理哪些 SxKernelSU 功能
- 防止多個模組嘗試管理同一功能時發生衝突
- 更好地協調模組與核心 SxKernelSU 功能之間的關係

::: warning 僅支援預定義功能
僅使用上面列出的預定義功能名稱(`su_compat`、`kernel_umount`)。這些對應於實際的 SxKernelSU 內部功能。使用其他功能名稱不會導致錯誤,但沒有任何功能作用。
:::
