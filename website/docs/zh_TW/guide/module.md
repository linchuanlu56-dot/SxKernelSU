...
SxKernelSU 使用 [metamodule](metamodule.md) 架構來掛載 `system` 目錄。**僅當您的模組需要修改 `/system` 檔案時**(透過 `system` 目錄)，您才需要安裝 metamodule(如 [meta-overlayfs](https://github.com/linchuanlu56-dot/SxKernelSU/releases))。其他模組功能如腳本、sepolicy 規則和 system.prop 無需 metamodule 即可運作。
:::
...
SxKernelSU 提供了一個完備的 BusyBox 二進位檔案 (包括完整的 SELinux 支援)。可執行檔位於 `/data/adb/sks/bin/busybox`。
...
1. 將環境變數 `ASH_STANDALONE` 設為 `1`。例如：`ASH_STANDALONE=1 /data/adb/sks/bin/busybox sh <script>`
2. 使用命令列選項切換：`/data/adb/sks/bin/busybox sh -o standalone <script>`
...
## SxKernelSU 模組 {#sxkernelsu-modules}
...
- `SKS_VER` (string): SxKernelSU 目前的版本名稱 (例如 `v0.4.0`)
- `SKS_VER_CODE` (int): SxKernelSU 使用者空間目前的版本代碼 (例如 `10672`)
- `SKS_KERNEL_VER_CODE` (int): SxKernelSU 核心空間目前的版本代碼 (例如 `10672`)
...
透過執行 `sksud late-load` 命令觸發。該命令會：

1. 偵測當前 KMI 版本，從內嵌資源中載入對應的 `sxkernelsu.ko`。
...
| sksud 的 kprobe 鉤子 (execve/read/fstat/input) | 是 | 跳過 |
| 安全模式偵測（音量鍵） | 是 | 始終停用 |
...
模組可以透過 `SKS_LATE_LOAD` 環境變數偵測當前是否處於 late-load 模式：
