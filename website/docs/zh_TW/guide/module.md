...
SxKernelSU 提供了一個模組機制，它可以在保持系統分割區完整性的同時達到修改系統分割區的效果；這種機制一般被稱為 systemless (無系統修改)。

SxKernelSU 的模組運作機制與 Magisk 幾乎相同，如果您熟悉 Magisk 模組的開發，那麼開發 SxKernelSU 的模組大同小異，您可以跳過下列有關模組的介紹，只需要瞭解 [SxKernelSU 模組與 Magisk 模組的差異](difference-with-magisk.md)。

::: warning METAMODULE 僅在修改系統檔案時需要
SxKernelSU 使用 [metamodule](metamodule.md) 架構來掛載 `system` 目錄。**僅當您的模組需要修改 `/system` 檔案時**(透過 `system` 目錄)，您才需要安裝 metamodule(如 [meta-overlayfs](https://github.com/tiann/SxKernelSU/releases))。其他模組功能如腳本、sepolicy 規則和 system.prop 無需 metamodule 即可運作。
:::

## WebUI

SxKernelSU 的模組支援顯示互動介面，請參閱 [WebUI 文檔](module-webui.md).

## 模組配置

SxKernelSU 提供了一個內建的配置系統，允許模組儲存持久化或暫時的鍵值設定。詳情請參閱[模組配置文檔](module-config.md)。

## Busybox

SxKernelSU 提供了一個完備的 BusyBox 二進位檔案 (包括完整的 SELinux 支援)。可執行檔位於 `/data/adb/ksu/bin/busybox`。
SxKernelSU 的 BusyBox 支援執行時可切換的 "ASH 獨立模式"。
...
每個基於 SxKernelSU 上下文的腳本都將在 BusyBox 的獨立模式執行。對於第三方開發人員而言，這包括所有開機腳本和模組安裝腳本。

對於想要在 SxKernelSU 之外使用這個「獨立模式」功能的使用者，有兩種啟用方法：
...
為了確保所有後續的 `sh` shell 都在獨立模式下執行，第一種是首選方法 (這也是 SxKernelSU 和 SxKernelSU 管理器內部使用的方法)，因為環境變數會被繼承到子執行緒中。

::: tip 與 Magisk 的差異
SxKernelSU 的 BusyBox 現在是直接使用 Magisk 專案編譯的二進位檔案，**感謝 Magisk！**
因此，您完全不必擔心 BusyBox 腳本與在 Magisk 和 SxKernelSU 之間的相容性問題，因為它們完全相同！
:::

## SxKernelSU 模組 {#kernelsu-modules}

SxKernelSU 模組是一個放置於 `/data/adb/modules` 且滿足下列結構的資料夾：
...
│   │      *** 狀態旗標 ***
│   │
│   ├── skip_mount          <--- 如果這個檔案存在，那麼 SxKernelSU 將不會掛接您的系統資料夾
...
|   ├── uninstall.sh        <--- 這個腳本將會在 SxKernelSU 移除模組時執行
...
::: tip 與 Magisk 的差異
SxKernelSU 沒有內建的針對 Zygisk 的支援，因此模組中沒有與 Zygisk 相關的內容，但您可以透過安裝 [ZygiskNext](https://github.com/Dr-TSNG/ZygiskNext) 以支援 Zygisk 模組，此時 Zygisk 模組的內容與 Magisk 所支援的 Zygisk 完全相同。
:::

### module.prop

module.prop 是一個模組的設定檔，在 SxKernelSU 中如果模組中不包含這個檔案，那麼它將不被認為是一個模組；這個檔案的格式如下：
...
:::tip 與 Magisk 的差異
您可以透過環境變數 `KSU` 來判斷腳本是執行在 SxKernelSU 還是 Magisk 中，如果執行在 SxKernelSU，這個值會被設為 `true`。
:::
...
您也可以在 `customize.sh` 中宣告一個名為 `REMOVE` 並且包含一系列目錄的變數以執行移除作業，SxKernelSU 會自動為您在模組對應目錄執行 `mknod <TARGET> c 0 0`。例如：
...
您可以在 `customize.sh` 中宣告一個名為 `REMOVE` 並且包含一系列目錄的變數以執行移除作業，SxKernelSU 會自動為您在模組對應目錄執行相關作業。例如：
...
SxKernelSU 的 systemless 機制透過核心的 overlayfs 實作，而 Magisk 目前則是透過 magic mount (bind mount)，兩者的實作方式有很大的差別，但最終的目標是一致的：不修改實際的 `/system` 分區但修改 `/system` 檔案。
:::
...
SxKernelSU 提供了一種將自訂 Android Init RC 指令注入系統 `init.rc` 的機制。這使得模組可以在不修改系統分區的情況下註冊自訂的 Android 服務、設定屬性觸發器或執行其他 Init 語言操作。

在啟動過程中，SxKernelSU 的核心模組透過 hook `read()` 和 `fstat()` 系統調用，在 Android init 進程讀取 `/system/etc/init/hw/init.rc` 時，將自訂 RC 內容透明地附加到文件末尾。init 進程會像處理原始 init.rc 內容一樣解析這些注入的指令。
...
SxKernelSU 的模組安裝程式就是一個可以透過 SxKernelSU 管理員應用程式刷新的 Zip 檔案，這個 Zip 檔案的格式如下：
...
:::warning 警告
SxKernelSU 模組不支援在 Recovery 中安裝！！
:::
...
`customize.sh` 腳本以「獨立模式」執行在 SxKernelSU 的 BusyBox `ash` shell 中。您可以使用下列變數和函式：

#### 變數 {#variables}

- `KSU` (bool): 標示此腳本執行於 SxKernelSU 環境中，此變數的值將永遠為 `true`，您可以透過它與 Magisk 進行區分。
- `KSU_VER` (string): SxKernelSU 目前的版本名稱 (例如 `v0.4.0`)
- `KSU_VER_CODE` (int): SxKernelSU 使用者空間目前的版本代碼 (例如 `10672`)
- `KSU_KERNEL_VER_CODE` (int): SxKernelSU 核心空間目前的版本代碼 (例如 `10672`)
- `BOOTMODE` (bool): 此變數在 SxKernelSU 中永遠為 `true`
...
- `KSU_UAPI_VER` (int): SxKernelSU 使用者空間 (ksud) 的 UAPI 版本號 (例如 `2`)。當核心驅動發生破壞性更改時此版本號會遞增，模組可據此判斷相容性。
- `KSU_RUNTIME_MODE` (string): SxKernelSU 目前的執行模式。可能的值為 `built-in`（即 GKI 模式，編譯進核心）、`lkm`（開機時作為核心模組載入）或 `late-load`（開機後作為核心模組載入）。
- `KSU_LATE_LOAD` (int?): 如果 SxKernelSU 是在開機後延遲載入的，此變數的值為 `1`，否則不設定此變數。

::: warning 警告
`MAGISK_VER_CODE` 在 SxKernelSU 永遠為 `25200`，`MAGISK_VER` 則為 `v25.2`，請不要透過這兩個變數來判斷是否為 SxKernelSU！
:::
...
在 SxKernelSU 中，依據腳本執行模式的不同分為兩種：post-fs-data 模式和 late_start 服務模式。
...
在 SxKernelSU 中，開機腳本依據存放位置的不同還分為兩種：一般腳本和模組腳本。
...
所有啟動腳本都將在 SxKernelSU 的 BusyBox ash shell 中執行，並啟用**獨立模式**。

### 開機腳本解釋 {#boot-scripts-process-explanation}

以下是Android的相關啟動流程（部分省略），其中包括SxKernelSU的運行（帶前導星號），可以幫助您更好地理解這些模組腳本的用途：
```txt
...
除了上述標準啟動流程外，SxKernelSU 還支援 **late-load 模式**，用於 LKM（可載入核心模組）場景。在該模式下，SxKernelSU 核心模組在**系統完全啟動後**載入，而非在 init 過程中載入。
