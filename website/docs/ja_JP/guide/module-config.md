...
SxKernelSU は、モジュールが永続的または一時的なキー値設定を保存できる組み込みの設定システムを提供します。設定は `/data/adb/sks/module_configs/<module_id>/` にバイナリ形式で保存され、以下の特性があります:
...
すべてのモジュールスクリプト(`post-fs-data.sh`、`service.sh`、`boot-completed.sh` など)は、`SKS_MODULE` 環境変数がモジュール ID に設定された状態で実行されます。`sksud module config` コマンドを使用してモジュールの設定を管理できます:
...
- 設定の問題をデバッグするには `sksud module config list` コマンドを使用
:::
...
```bash
# モジュール説明をオーバーライド
sksud module config set override.description "マネージャーに表示されるカスタム説明"
```
...
モジュールは `manage.<feature>` 設定パターンを使用して、管理する SxKernelSU 機能を宣言できます。サポートされている機能は、SxKernelSU 内部の `FeatureId` 列挙型に対応しています:
...
- SxKernelSU マネージャーがどのモジュールがどの SxKernelSU 機能を管理しているかを検出
- 複数のモジュールが同じ機能を管理しようとした際の競合を防止
- モジュールとコア SxKernelSU 機能間のより良い調整

::: warning サポートされている機能のみ
上記にリストされた事前定義された機能名(`su_compat`、`kernel_umount`)のみを使用してください。これらは実際の SxKernelSU 内部機能に対応しています。他の機能名を使用してもエラーにはなりませんが、機能的な目的はありません。
:::
