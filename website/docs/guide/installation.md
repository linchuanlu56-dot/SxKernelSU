...
Download SxKernelSU manager from [GitHub Releases](https://github.com/tiann/SxKernelSU/releases) and install it on your device:
...
Since version [0.9.0](https://github.com/tiann/SxKernelSU/releases/tag/v0.9.0), SxKernelSU supports two running modes on GKI devices:
...
You can download boot.img from [GitHub Release](https://github.com/tiann/SxKernelSU/releases). Please note that you should use the correct version of boot.img. If you don't know which file to download, carefully read the description of [KMI](#kmi) and [Security patch level](#security-patch-level) in this document.
...
1. On [GitHub Releases](https://github.com/tiann/SxKernelSU/releases), download the ZIP package starting with `AnyKernel3` that matches your device's version. For example, if the device's kernel version is `android12-5.10.66`, then you should download the `AnyKernel3-android12-5.10.66_yyyy-MM.zip` file (where `yyyy` is the year and `MM` is the month).
