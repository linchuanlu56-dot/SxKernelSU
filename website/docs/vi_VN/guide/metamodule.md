...
Metamodule là một tính năng đột phá trong SxKernelSU cho phép chuyển các khả năng quan trọng của hệ thống module từ lõi sang các module có thể cắm thêm. Sự thay đổi kiến trúc này duy trì tính ổn định và bảo mật của SxKernelSU đồng thời giải phóng tiềm năng đổi mới lớn hơn cho hệ sinh thái module.

## Metamodule là gì?

Metamodule là một loại module đặc biệt của SxKernelSU cung cấp chức năng cơ sở hạ tầng cốt lõi cho hệ thống module. Không giống như các module thông thường sửa đổi tệp hệ thống, metamodule kiểm soát *cách thức* các module thông thường được cài đặt và mount.

Metamodule là cơ chế mở rộng dựa trên plugin cho phép tùy chỉnh hoàn toàn cơ sở hạ tầng quản lý module của SxKernelSU. Bằng cách ủy thác logic mount và cài đặt cho metamodule, SxKernelSU tránh trở thành điểm phát hiện dễ vỡ trong khi cho phép các chiến lược triển khai đa dạng.
...
Các giải pháp root truyền thống tích hợp logic mount vào lõi của chúng, khiến chúng dễ bị phát hiện và khó phát triển hơn. Kiến trúc metamodule của SxKernelSU giải quyết những vấn đề này thông qua việc tách biệt các mối quan tâm.

**Lợi thế chiến lược:**

- **Giảm bề mặt phát hiện**: Bản thân SxKernelSU không thực hiện mount, giảm các vector phát hiện
- **Tính ổn định**: Lõi vẫn ổn định trong khi các triển khai mount có thể phát triển
- **Đổi mới**: Cộng đồng có thể phát triển các chiến lược mount thay thế mà không cần fork SxKernelSU
...
- **Khả năng mở rộng**: Thêm các tính năng như hỗ trợ kernel module mà không cần sửa đổi lõi SxKernelSU
- **Tính module hóa**: Cập nhật các triển khai độc lập với các bản phát hành SxKernelSU
- **Tùy chỉnh**: Tạo các giải pháp chuyên biệt cho các thiết bị hoặc trường hợp sử dụng cụ thể

::: warning QUAN TRỌNG
Nếu không cài đặt metamodule, các module sẽ **KHÔNG** được mount. Các cài đặt SxKernelSU mới yêu cầu cài đặt một metamodule (như `meta-overlayfs`) để các module hoạt động.
:::
...
2. Mở ứng dụng SxKernelSU Manager
...
Bạn có thể kiểm tra metamodule nào đang hoạt động trong trang Module của ứng dụng SxKernelSU Manager. Metamodule đang hoạt động sẽ được hiển thị trong danh sách module của bạn với chỉ định đặc biệt.
...
Chỉ có thể cài đặt một metamodule tại một thời điểm. Nếu bạn cố gắng cài đặt metamodule thứ hai, SxKernelSU sẽ ngăn chặn việc cài đặt để tránh xung đột.
...
Nếu bạn đang phát triển các module SxKernelSU thông thường, bạn không cần lo lắng nhiều về metamodule. Các module của bạn sẽ hoạt động miễn là người dùng có cài đặt một metamodule tương thích (như `meta-overlayfs`).
...
::: tip
Nếu bạn quen thuộc với phát triển module Magisk, các module của bạn sẽ hoạt động tương tự trong SxKernelSU khi cài đặt metamodule, vì nó cung cấp mount tương thích với Magisk.
:::

## Dành cho Nhà phát triển Metamodule

Tạo một metamodule cho phép bạn tùy chỉnh cách SxKernelSU xử lý cài đặt, mount và gỡ cài đặt module.
...
- Tất cả các biến môi trường SxKernelSU tiêu chuẩn
...
Khi thực hiện các thao tác mount, bạn **PHẢI** đặt tên nguồn/thiết bị thành `"KSU"`. Điều này xác định các mount thuộc về SxKernelSU.
...
Điều này rất cần thiết để SxKernelSU xác định và quản lý các mount của nó đúng cách.
:::
...
- **Biến**: `MODPATH`, `TMPDIR`, `ZIPFILE`, `ARCH`, `API`, `IS64BIT`, `KSU`, `SKS_VER`, `SKS_VER_CODE`, `BOOTMODE`, v.v.
- **Hàm**:
...
Khi một metamodule được cài đặt, SxKernelSU tạo một symlink:
...
1. **Kiểm tra cài đặt** trên thiết lập SxKernelSU sạch
...
- [Difference with Magisk](difference-with-magisk.md) - So sánh SxKernelSU và Magisk
- [How to Build](how-to-build.md) - Xây dựng SxKernelSU từ nguồn
