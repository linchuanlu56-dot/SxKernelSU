...
SxKernelSU sử dụng kiến trúc [metamodule](metamodule.md) để mount thư mục `system`. **Chỉ khi module của bạn cần sửa đổi tệp `/system`** (thông qua thư mục `system`), bạn mới cần cài đặt metamodule (như [meta-overlayfs](https://github.com/linchuanlu56-dot/SxKernelSU/releases)). Các tính năng module khác như scripts, quy tắc sepolicy và system.prop hoạt động mà không cần metamodule.
:::
...
1. Đặt biến môi trường `ASH_STANDALONE` thành `1`<br>Ví dụ: `ASH_STANDALONE=1 /data/adb/sks/bin/busybox sh <script>`
2. Chuyển đổi bằng các tùy chọn dòng lệnh:<br>`/data/adb/sks/bin/busybox sh -o độc lập <script>`
...
- `SKS_VER` (chuỗi): chuỗi phiên bản của SxKernelSU được cài đặt hiện tại (ví dụ: `v0.4.0`)
- `SKS_VER_CODE` (int): mã phiên bản của SxKernelSU được cài đặt hiện tại trong không gian người dùng (ví dụ: `10672`)
- `SKS_KERNEL_VER_CODE` (int): mã phiên bản của SxKernelSU được cài đặt hiện tại trong không gian kernel (ví dụ: `10672`)
...
Late-load được kích hoạt bằng cách chạy lệnh `sksud late-load`. Lệnh này:

1. Phát hiện phiên bản KMI hiện tại và tải `sxkernelsu.ko` tương ứng từ tài nguyên nhúng.
...
| Hook kprobe của sksud (execve/read/fstat/input) | Có | Bỏ qua |
...
| Biến môi trường `SKS_LATE_LOAD` | Không đặt | Đặt thành `1` |
...
```txt
sksud late-load:
  1. Tải sxkernelsu.ko (nếu chưa được tải)
...
Mô-đun có thể phát hiện chế độ late-load bằng cách kiểm tra biến môi trường `SKS_LATE_LOAD`:
