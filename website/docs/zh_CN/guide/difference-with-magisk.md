# SxKernelSU 模块与 Magisk 的差异 {#title}

虽然 SxKernelSU 模块与 Magisk 模块有很多相似之处，但由于它们的实现机制完全不同，因此不可避免地会有一些差异；如果你希望你的模块能同时在 Magisk 与 SxKernelSU 中运行，那么你必须了解这些差异。
...
在了解不同之处之前，你需要知道如何区分你的模块是运行在 SxKernelSU 还是运行在 Magisk 之中；在所有你可以运行模块脚本的地方（`customize.sh`, `post-fs-data.sh`, `service.sh`)，你都可以通过环境变量`KSU` 来区分，在 SxKernelSU 中，这个环境变量将被设置为 `true`。

以下是一些不同之处：

1. SxKernelSU 的模块不支持在 Recovery 中安装。
