...
从 [GitHub Releases](https://github.com/linchuanlu56-dot/SxKernelSU/releases) 下载 SxKernelSU 管理器应用，然后将应用程序安装到设备并打开：

- 如果应用程序显示 “不支持”，则表示您的设备不支持 SxKernelSU，你需要自己编译设备的内核才能使用，SxKernelSU 官方不会也永远不会为你提供一个可以刷写的 boot 镜像。
- 如果应用程序显示 “未安装”，那么 SxKernelSU 支持您的设备；可以进行下一步操作。
...
自 `0.9.0` 版本以后，在 GKI 设备中，SxKernelSU 支持两种运行模式：
...
GKI 模式会替换掉设备原有的内核，使用 SxKernelSU 提供的通用内核镜像。GKI 模式的优点是：
...
1. 不会替换掉设备原有的内核；如果你对设备原有的内核有特殊需求，或者你希望在使用第三方内核的同时使用 SxKernelSU，可以使用 LKM 模式；
2. 升级和 OTA 较为方便；升级 SxKernelSU 时，可以直接在管理器里面安装，无需再手动刷写；系统 OTA 后，可以直接安装到第二个槽位，也无需再手动刷写；
...
获取官方固件的方法有很多，如果你的设备支持 `fastboot boot`，那么我们最推荐以及最简单的方法是使用 `fastboot boot` 临时启动 SxKernelSU 提供的 GKI 内核，然后安装管理器，最后在管理器中直接安装；这种方法不需要你手动下载官方固件，也不需要你手动提取 boot。
...
2. 直接安装；如果你手机已经 root，你可以选择这个选项，管理器会自动获取你的设备信息，然后自动修补官方固件，然后刷入；你可以考虑使用 `fastboot boot` SxKernelSU 的 GKI 内核来获取临时 root 安装管理器，然后再使用这个选项；这种方式也是 SxKernelSU 升级最主要的方式；
3. 安装到另一个分区；如果你的设备支持 A/B 分区，你可以选择这个选项，管理器会自动修补官方固件，然后安装到另一个分区；这种方式适用于 OTA 后的设备，你可以在 OTA 后直接安装到另一个分区，然后重启设备即可；

### 使用命令行

如果你不想使用管理器，你也可以使用命令行来安装 LKM；SxKernelSU 提供的 `sksud` 工具可以帮助你快速修补官方固件，然后刷入。

这个工具支持 macOS、Linux 和 Windows，你可以在 [GitHub Release](https://github.com/linchuanlu56-dot/SxKernelSU/releases) 下载对应的版本。

使用方法：`sksud boot-patch` 具体的使用方法你可以查看命令行帮助。
...
1. `--magiskboot` 选项可以指定 magiskboot 的路径，如果不指定，sksud 会在环境变量中查找；如果你不知道如何获取 magiskboot，可以查阅[这里](#patch-boot-image)；
...
1. 使用 SxKernelSU 提供的**通用内核镜像**使用 fastboot 安装
2. 使用内核刷写 App（如 KernelFlasher）安装
3. 手动修补 boot.img 然后安装
4. 使用自定义 Recovery（如 TWRP）安装

## 使用 SxKernelSU 提供的 boot.img 安装 {#install-by-sxkernelsu-boot-image}

如果你设备的 `boot.img` 采用常用的压缩格式，那么可以采用 SxKernelSU 提供的的通用内核镜像直接刷入，它不需要 TWRP 或者自行修补镜像。

### 找到合适的 boot.img {#found-propery-image}

SxKernelSU 为 GKI 设备提供了通用的 boot.img，您应该将 boot.img 刷写到设备的 boot 分区。

您可以从 [GitHub Release](https://github.com/linchuanlu56-dot/SxKernelSU/releases) 下载 boot.img, 请注意您应该使用正确版本的 boot.img。如果您不知道应该下载哪一个文件，请仔细阅读本文档中关于 [KMI](#kmi) 和[安全补丁级别](#security-patch-level)的描述。
...
使用 `adb` 连接您的设备，然后执行 `adb reboot bootloader` 进入 fastboot 模式，然后使用此命令刷入 SxKernelSU：
...
1. 你的设备已经获取了 root 权限，比如你已经安装好了 SxKernelSU 想升级到最新的版本，又或者你通过其他方法（如 Magisk）获取了 root。
2. 如果你的手机没有 root，但手机支持 `fastboot boot boot.img` 这种临时启动的方法，你可以用 SxKernelSU 提供的 GKI 镜像临时启动你的设备，获取临时的 root 权限，然后使用内核刷写器刷入获取永久 root 权限。
...
对于某些设备来说，其 boot.img 格式不那么常见，比如不是 `lz4`, `gz` 和未压缩；最典型的就是 Pixel，它 boot.img 的格式是 `lz4_legacy` 压缩，ramdisk 可能是 `gz` 也可能是 `lz4_legacy` 压缩；此时如果你直接刷入 SxKernelSU 提供的 boot.img，手机可能无法开机；这时候，你可以通过手动修补 boot.img 来实现。
...
2. 下载 SxKernelSU 提供的与你设备 KMI 版本一致的 AnyKernel3 刷机包；如果您不知道应该下载哪一个文件，请仔细阅读本文档中关于 [KMI](#kmi) 和[安全补丁级别](#security-patch-level)的描述。
3. 解压缩 AnyKernel3 刷机包，获取其中的 `Image` 文件，此文件为 SxKernelSU 的内核文件。
...
2. 在你的 PC 上准备好设备原厂的 boot.img 和 SxKernelSU 的 Image。
...
1. 在 SxKernelSU 的 [Release 页面](https://github.com/linchuanlu56-dot/SxKernelSU/releases) 下载与你手机版本匹配的以 AnyKernel3 开头的 zip 刷机包；如果你不知道下载哪一个，请仔细查阅上述文档中关于 **KMI** 和**安全补丁级别**的描述；下载错误的刷机包很可能导致无法开机，请注意备份。
2. 重启手机进入 TWRP。
...
其实所有这些安装方法的主旨只有一个，那就是**替换原厂的内核为 SxKernelSU 提供的内核**；只要能实现这个目的，就可以安装；比如以下是其他可行的方法：

1. 首先安装 Magisk，通过 Magisk 获取 root 权限后使用内核刷写器刷入 SxKernelSU 的 AnyKernel 包。
2. 使用某些 PC 上的刷机工具箱刷入 SxKernelSU 提供的内核。
...
如果您想使用修改 `/system` 文件的模块，需要在安装 SxKernelSU 后安装一个 **metamodule**。仅使用脚本、sepolicy 或 system.prop 的模块无需 metamodule。
:::
