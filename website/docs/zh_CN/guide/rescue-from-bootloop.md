...
如果大家刷入某些开源的或者被证明是安全的模块使得手机无法启动，那么这种情况在 SxKernelSU 中非常容易恢复，完全无需担心。SxKernelSU 内置了如下两种机制来救砖：
...
SxKernelSU 的模块更新借鉴了 Android 系统 OTA 更新时的 AB 更新机制，如果你安装了新模块或者对已有模块有更新操作，不会直接操作当前使用的模块文件，而是会把所有模块构建成另外一个 update 镜像；系统重启之后，会使用这个 update 镜像尝试启动一次，如果 Android 系统成功启动，才会真正更新模块。
