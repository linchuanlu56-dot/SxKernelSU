...
SxKernelSU 提供了一个内置的配置系统,允许模块存储持久化或临时的键值设置。配置以二进制格式存储在 `/data/adb/sks/module_configs/<module_id>/`,具有以下特性:
...
所有模块脚本(`post-fs-data.sh`、`service.sh`、`boot-completed.sh` 等)运行时都会设置 `SKS_MODULE` 环境变量为模块 ID。您可以使用 `sksud module config` 命令来管理模块的配置:
...
- 在脚本中使用配置值之前验证它们
- 使用 `sksud module config list` 命令调试配置问题
:::
...
```bash
# 覆盖模块描述
sksud module config set override.description "在管理器中显示的自定义描述"
```
...
模块可以使用 `manage.<feature>` 配置模式声明它们管理的 SxKernelSU 功能。支持的功能对应于 SxKernelSU 内部的 `FeatureId` 枚举:
...
- SxKernelSU 管理器检测哪些模块管理哪些 SxKernelSU 功能
- 防止多个模块尝试管理同一功能时发生冲突
- 更好地协调模块与核心 SxKernelSU 功能之间的关系

::: warning 仅支持预定义功能
仅使用上面列出的预定义功能名称(`su_compat`、`kernel_umount`)。这些对应于实际的 SxKernelSU 内部功能。使用其他功能名称不会导致错误,但没有任何功能作用。
:::
