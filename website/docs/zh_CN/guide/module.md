...
SxKernelSU 使用 [metamodule](metamodule.md) 架构来挂载 `system` 目录。**只有当您的模块需要修改 `/system` 文件**（通过 `system` 目录）时，才需要安装 metamodule（例如 [meta-overlayfs](https://github.com/linchuanlu56-dot/SxKernelSU/releases)）。其他模块功能如脚本、sepolicy 规则和 system.prop 无需 metamodule 即可工作。
:::
...
SxKernelSU 提供了一个功能完备的 BusyBox 二进制文件（包括完整的 SELinux 支持）。可执行文件位于 `/data/adb/sks/bin/busybox`。
...
1. 设置环境变量 `ASH_STANDALONE` 为 `1`。例如：`ASH_STANDALONE=1 /data/adb/sks/bin/busybox sh <script>`
2. 使用命令行选项切换：`/data/adb/sks/bin/busybox sh -o standalone <script>`
...
## SxKernelSU 模块 {#sxkernelsu-modules}
...
- `SKS_VER` (string): SxKernelSU 当前的版本名字 (如： `v0.4.0`)
- `SKS_VER_CODE` (int): SxKernelSU 用户空间当前的版本号 (如. `10672`)
- `SKS_KERNEL_VER_CODE` (int): SxKernelSU 内核空间当前的版本号 (如. `10672`)
...
通过运行 `sksud late-load` 命令触发。该命令会：

1. 检测当前 KMI 版本，从内嵌资源中加载对应的 `sxkernelsu.ko`。
...
| sksud 的 kprobe 钩子 (execve/read/fstat/input) | 是 | 跳过 |
| 安全模式检测（音量键） | 是 | 始终禁用 |
...
模块可以通过 `SKS_LATE_LOAD` 环境变量检测当前是否处于 late-load 模式：
