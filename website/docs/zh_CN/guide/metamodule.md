...
元模块是 SxKernelSU 的一项革新性功能，它将关键的模块系统能力从核心转移到可插拔模块中。这种架构转变在保持 SxKernelSU 稳定性和安全性的同时，为模块生态系统释放了更大的创新潜力。

## 什么是元模块?

元模块是一种特殊类型的 SxKernelSU 模块，为模块系统提供核心基础设施功能。与常规模块不同，元模块控制常规模块的*安装和挂载方式*。

元模块是一种基于插件的扩展机制，允许完全自定义 SxKernelSU 的模块管理基础设施。通过将挂载和安装逻辑委托给元模块，SxKernelSU 避免成为脆弱的检测点，同时支持多样化的实现策略。
...
传统的 Root 解决方案将挂载逻辑内置在核心中，这使得它们更容易被检测且难以演进。SxKernelSU 的元模块架构通过关注点分离解决了这些问题。

**战略优势:**

- **减少检测面**: SxKernelSU 本身不执行挂载，减少了检测向量
...
- **可扩展性**: 添加内核模块支持等功能，无需修改核心 SxKernelSU
- **模块化**: 独立于 SxKernelSU 版本更新实现
- **定制化**: 为特定设备或用例创建专门的解决方案

::: warning 重要
如果没有安装元模块，依赖挂载的模块，其挂载功能将不会生效。新安装的 SxKernelSU 需要安装元模块(如 `meta-overlayfs`)才能使模块正常工作。
:::
...
您可以在 SxKernelSU Manager 应用的模块页面中查看当前活动的元模块。活动的元模块将显示在模块列表中，并带有特殊标识。
...
只允许一个元模块处于运行状态。如果您尝试安装第二个元模块，SxKernelSU 将阻止安装以避免冲突。
...
如果您正在开发常规 SxKernelSU 模块，您不需要太担心元模块。只要用户安装了兼容的元模块(如 `meta-overlayfs`)，您的模块就能正常工作。
...
::: tip
如果您熟悉 Magisk 模块开发，您的模块在安装元模块后将在 SxKernelSU 中以相同方式工作，因为它提供了 Magisk 兼容的挂载。
:::

## 对于元模块开发者

创建元模块允许您自定义 SxKernelSU 处理模块安装、挂载和卸载的方式。
...
::: danger 关键要求
执行挂载操作时，**必须**将源/设备名称设置为 `"KSU"`。这将挂载标识为属于 SxKernelSU。
...
- **变量**: `MODPATH`、`TMPDIR`、`ZIPFILE`、`ARCH`、`API`、`IS64BIT`、`KSU`、`SKS_VER`、`SKS_VER_CODE`、`BOOTMODE` 等
- **函数**:
...
- [与 Magisk 的区别](difference-with-magisk.md) - 比较 SxKernelSU 和 Magisk
- [如何构建](how-to-build.md) - 从源代码构建 SxKernelSU
