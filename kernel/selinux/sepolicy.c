...
#define sksu_hash_for_each(node_ptr, n_slot, cur)                                                                       \
    int i;                                                                                                             \
    for (i = 0; i < n_slot; ++i)                                                                                       \
...
#define sksu_hashtab_for_each(htab, cur) sksu_hash_for_each(htab.htable, htab.size, cur)
#else
#define sksu_hashtab_for_each(htab, cur) sksu_hash_for_each(htab->htable, htab->size, cur)
#endif
...
#define avtab_for_each(avtab, cur) sksu_hash_for_each(avtab.htable, avtab.nslot, cur);
                         ...
                         struct perm_datum *perm, int effect, bool invert)
{
    ...
    if (src == NULL) {
        ...
        if (strip_av(effect, invert)) {
            sksu_hashtab_for_each(db->p_types.table, node)
            {
        ...
        struct hashtab_node *node;
        sksu_hashtab_for_each(db->p_classes.table, node)
        {
...
#define sksu_kvrealloc(p, new_size, _old_size) kvrealloc(p, new_size, GFP_KERNEL)
...
#define sksu_kvrealloc(p, new_size, old_size) kvrealloc(p, old_size, new_size, GFP_KERNEL)
#else
...
void *sksu_kvrealloc_compat(const void *p, size_t oldsize, size_t newsize, gfp_t flags)
{
    void *newp;
...
#define sksu_kvrealloc(p, new_size, old_size) sksu_kvrealloc_compat(p, old_size, new_size, GFP_KERNEL)
#endif
...
static bool add_type(struct policydb *db, const char *type_name, bool attr)
{
    ...
    struct ebitmap *new_type_attr_map_array =
        sksu_kvrealloc(db->type_attr_map_array, value * sizeof(struct ebitmap), (value - 1) * sizeof(struct ebitmap));
    ...
    struct type_datum **new_type_val_to_struct = sksu_kvrealloc(
    ...
    char **new_val_to_name_types =
        sksu_kvrealloc(db->sym_val_to_name[SYM_TYPES], sizeof(char *) * value, sizeof(char *) * (value - 1));
...
void sksu_destroy_sepolicy(struct selinux_policy *pol)
{
