#ifndef __SKS_H_LSM_HOOK
#define __SKS_H_LSM_HOOK
...
#define SKS_LSM_HOOK_HEADS_TYPE struct lsm_static_calls_table
#else
#define SKS_LSM_HOOK_HEADS_TYPE struct security_hook_heads
#endif

struct sksu_lsm_hook {
...
#define SKS_LSM_HOOK_INIT(member, target_symbol, replacement_fn, off)                                                  \
    {                                                                                                                  \
        .head_name = #member, .target_name = target_symbol, .head_offset = offsetof(SKS_LSM_HOOK_HEADS_TYPE, member),  \
...
// runtime lock for security_hook_heads mutations; callers rely on SxKernelSU's
...
// for restoration by sksu_lsm_unhook() or the global sksu_lsm_hook_exit().
int sksu_lsm_hook(struct sksu_lsm_hook *hook);

// Restore the LSM hook entry previously patched by sksu_lsm_hook() back to the
...
// after SxKernelSU-specific logic.
int sksu_register_lsm_hook(struct sksu_lsm_hook *hook);

// Undo sksu_register_lsm_hook() and restore the original BPF LSM handler.
