...
struct sksu_event_queue_node {
    struct list_head list;
    struct sksu_event_record_hdr hdr;
    __u8 payload[];
};

static size_t sksu_event_queue_record_size(__u32 payload_len)
{
    return sizeof(struct sksu_event_record_hdr) + payload_len;
}

static void sksu_event_queue_note_drop_locked(struct sksu_event_queue *queue, __u64 seq)
{
    queue->dropped_total++;
...
static bool sksu_event_queue_has_data_locked(const struct sksu_event_queue *queue)
{
    return queue->dropped_pending || queue->dropped_inflight || !list_empty(&queue->pending);
}

static void sksu_event_queue_mark_closed(struct sksu_event_queue *queue)
{
...
{
    struct sksu_event_queue_node *node, *tmp;
    unsigned long irq_flags;

    sksu_event_queue_mark_closed(queue);
...
int sksu_event_queue_push(struct sksu_event_queue *queue, __u16 type, __u16 flags, const void *payload, __u32 len,
                         gfp_t gfp)
{
    struct sksu_event_queue_node *node = NULL;
    unsigned long irq_flags;
    __u64 seq;
    bool wake = false;
    int ret = 0;
    ...
    if (!node || (queue->max_queued && queue->queued >= queue->max_queued)) {
        sksu_event_queue_note_drop_locked(queue, seq);
        wake = true;
...
static int sksu_event_queue_wait_ready(struct sksu_event_queue *queue, int file_flags)
{
    int ret;

    for (;;) {
        if (sksu_event_queue_has_data(queue)) {
            return 0;
        }
        ...
        ret = wait_event_interruptible(queue->read_wait, queue->closed || sksu_event_queue_has_data(queue));
        if (ret) {
            return ret;
        }
    }
...
static ssize_t sksu_event_queue_read_drop(struct sksu_event_queue *queue, char __user *buf, size_t count)
{
    struct sksu_event_record_hdr hdr;
    struct sksu_event_queue_dropped_info info;
    size_t record_size = sksu_event_queue_record_size(sizeof(info));
    unsigned long irq_flags;
    ...
    hdr.type = SKS_EVENT_QUEUE_TYPE_DROPPED;
    hdr.flags = SKS_EVENT_RECORD_FLAG_INTERNAL;
    hdr.len = sizeof(info);
...
static ssize_t sksu_event_queue_read_node(struct sksu_event_queue *queue, char __user *buf, size_t count)
{
    struct sksu_event_queue_node *node;
    ...
    node = list_entry(first, struct sksu_event_queue_node, list);
    record_size = sksu_event_queue_record_size(node->hdr.len);
    if (count < record_size) {
        spin_unlock_irqrestore(&queue->lock, irq_flags);
        return -EMSGSIZE;
    }
...
ssize_t sksu_event_queue_read(struct sksu_event_queue *queue, char __user *buf, size_t count, int file_flags)
{
    ...
    ret = sksu_event_queue_wait_ready(queue, file_flags);
    if (ret) {
        copied = ret;
        goto out_unlock;
    }
    ...
    while (count > 0) {
        ret = sksu_event_queue_read_drop(queue, buf, count);
        if (ret < 0) {
            if (!copied) {
                copied = ret;
            }
        ...
        ret = sksu_event_queue_read_node(queue, buf, count);
        if (ret < 0) {
            if (!copied) {
                copied = ret;
            }
...
__poll_t sksu_event_queue_poll(struct sksu_event_queue *queue, struct file *file, poll_table *wait)
{
    ...
    spin_lock_irqsave(&queue->lock, irq_flags);
    if (sksu_event_queue_has_data_locked(queue)) {
...
bool sksu_event_queue_has_data(struct sksu_event_queue *queue)
{
    ...
    spin_lock_irqsave(&queue->lock, irq_flags);
    has_data = sksu_event_queue_has_data_locked(queue);
    spin_unlock_irqrestore(&queue->lock, irq_flags);
