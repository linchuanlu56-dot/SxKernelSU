...
mod android {
    ...
    pub fn ensure_binaries(ignore_if_exist: bool) -> anyhow::Result<()> {
        for file in Asset::iter() {
            if file == "sksuinit" || file.ends_with(".ko") {
                // don't extract sksuinit and kernel modules
                continue;
            }
            let asset =
        ...
        // Create resetprop -> sksud symlink (resetprop is now built into sksud)
        let resetprop_link = RESETPROP_PATH;
        let _ = std::fs::remove_file(resetprop_link);
        std::os::unix::fs::symlink("/data/adb/sksd", resetprop_link)?;
...
pub fn list_supported_kmi() -> std::vec::Vec<std::string::String> {
    ...
    for file in Asset::iter() {
        // kmi_name = "xxx_sxkernelsu.ko"
        if let Some(kmi) = file.strip_suffix("_sxkernelsu.ko") {
