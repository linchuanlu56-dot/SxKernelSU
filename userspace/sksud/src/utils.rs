...
use crate::{assets, boot_patch, defs, skcalls, module, restorecon};
...
pub fn is_safe_mode() -> bool {
    ...
    let safemode = skcalls::check_kernel_safemode();
...
fn link_sksud_to_bin() -> Result<()> {
    let sksu_bin = PathBuf::from(defs::DAEMON_PATH);
    let sksu_bin_link = PathBuf::from(defs::DAEMON_LINK_PATH);
    if sksu_bin.exists() && !sksu_bin_link.exists() {
        std::os::unix::fs::symlink(&sksu_bin, &sksu_bin_link)?;
    }
    Ok(())
}
...
pub fn install(libadbroot: Option<PathBuf>) -> Result<()> {
    ...
    restorecon::lsetfilecon(defs::DAEMON_PATH, restorecon::SKS_CON)?;
...
pub fn uninstall(package_name: &str) -> Result<()> {
    ...
    println!("- Uninstall SxKernelSU manager..");
    Command::new("pm")
...
pub fn detach_process_group(use_init_pgrp: bool) {
    if use_init_pgrp {
        if let Err(e) = skcalls::set_init_pgrp() {
