...
impl<'a> TryFrom<&'a NormalPerm<'a>> for Vec<AtomicStatement> {
    ...
    fn try_from(perm: &'a NormalPerm<'a>) -> Result<Self> {
        ...
        let subcmd = match perm.op {
            "allow" => crate::sks_uapi::SKS_SEPOLICY_SUBCMD_NORMAL_PERM_ALLOW,
            "deny" => crate::sks_uapi::SKS_SEPOLICY_SUBCMD_NORMAL_PERM_DENY,
            "auditallow" => crate::sks_uapi::SKS_SEPOLICY_SUBCMD_NORMAL_PERM_AUDITALLOW,
            "dontaudit" => crate::sks_uapi::SKS_SEPOLICY_SUBCMD_NORMAL_PERM_DONTAUDIT,
            _ => 0,
        };
                ...
                for &c in &perm.class {
                    for &p in &perm.perm {
                        result.push(AtomicStatement {
                            cmd: crate::sks_uapi::SKS_SEPOLICY_CMD_NORMAL_PERM,
                            subcmd,
                            sepol1: s.try_into()?,
                            sepol2: t.try_into()?,
                            sepol3: c.try_into()?,
...
impl<'a> TryFrom<&'a XPerm<'a>> for Vec<AtomicStatement> {
    ...
    fn try_from(perm: &'a XPerm<'a>) -> Result<Self> {
        let mut result = vec![];
        let subcmd = match perm.op {
            "allowxperm" => crate::sks_uapi::SKS_SEPOLICY_SUBCMD_XPERM_ALLOW,
            "auditallowxperm" => crate::sks_uapi::SKS_SEPOLICY_SUBCMD_XPERM_AUDITALLOW,
            "dontauditxperm" => crate::sks_uapi::SKS_SEPOLICY_SUBCMD_XPERM_DONTAUDIT,
            _ => 0,
        };
        for &s in &perm.source {
            for &t in &perm.target {
                for &c in &perm.class {
                    for &r in &perm.perm_set {
                        result.push(AtomicStatement {
                            cmd: crate::sks_uapi::SKS_SEPOLICY_CMD_XPERM,
                            subcmd,
                            sepol1: s.try_into()?,
                            sepol2: t.try_into()?,
                            sepol3: c.try_into()?,
...
impl<'a> TryFrom<&'a TypeState<'a>> for Vec<AtomicStatement> {
    ...
    fn try_from(perm: &'a TypeState<'a>) -> Result<Self> {
        let mut result = vec![];
        let subcmd = match perm.op {
            "permissive" => crate::sks_uapi::SKS_SEPOLICY_SUBCMD_TYPE_STATE_PERMISSIVE,
            "enforce" => crate::sks_uapi::SKS_SEPOLICY_SUBCMD_TYPE_STATE_ENFORCE,
            _ => 0,
        };
        for &t in &perm.stype {
            result.push(AtomicStatement {
                cmd: crate::sks_uapi::SKS_SEPOLICY_CMD_TYPE_STATE,
                subcmd,
                sepol1: t.try_into()?,
...
impl<'a> TryFrom<&'a Type<'a>> for Vec<AtomicStatement> {
    ...
    fn try_from(perm: &'a Type<'a>) -> Result<Self> {
        ...
        for &attr in &perm.attrs {
            result.push(AtomicStatement {
                cmd: crate::sks_uapi::SKS_SEPOLICY_CMD_TYPE,
                subcmd: 0,
    ...
    fn try_from(perm: &'a TypeAttr<'a>) -> Result<Self> {
        ...
        for &t in &perm.stype {
            for &attr in &perm.sattr {
                result.push(AtomicStatement {
                    cmd: crate::sks_uapi::SKS_SEPOLICY_CMD_TYPE_ATTR,
                    subcmd: 0,
                    sepol1: t.try_into()?,
                    sepol2: attr.try_into()?,
                    sepol3: PolicyObject::None,
...
impl<'a> TryFrom<&'a Attr<'a>> for Vec<AtomicStatement> {
    ...
    fn try_from(perm: &'a Attr<'a>) -> Result<Self> {
        let result = vec![AtomicStatement {
            cmd: crate::sks_uapi::SKS_SEPOLICY_CMD_ATTR,
            subcmd: 0,
            sepol1: perm.name.try_into()?,
            sepol2: PolicyObject::None,
            sepol3: PolicyObject::None,
...
impl<'a> TryFrom<&'a TypeTransition<'a>> for Vec<AtomicStatement> {
    ...
    fn try_from(perm: &'a TypeTransition<'a>) -> Result<Self> {
        ...
        result.push(AtomicStatement {
            cmd: crate::sks_uapi::SKS_SEPOLICY_CMD_TYPE_TRANSITION,
            subcmd: 0,
...
impl<'a> TryFrom<&'a TypeChange<'a>> for Vec<AtomicStatement> {
    ...
    fn try_from(perm: &'a TypeChange<'a>) -> Result<Self> {
        let mut result = vec![];
        let subcmd = match perm.op {
            "type_change" => crate::sks_uapi::SKS_SEPOLICY_SUBCMD_TYPE_CHANGE_CHANGE,
            "type_member" => crate::sks_uapi::SKS_SEPOLICY_SUBCMD_TYPE_CHANGE_MEMBER,
            _ => 0,
        };
        result.push(AtomicStatement {
            cmd: crate::sks_uapi::SKS_SEPOLICY_CMD_TYPE_CHANGE,
            subcmd,
            sepol1: perm.source.try_into()?,
            sepol2: perm.target.try_into()?,
            sepol3: perm.class.try_into()?,
...
impl<'a> TryFrom<&'a GenFsCon<'a>> for Vec<AtomicStatement> {
    ...
    fn try_from(perm: &'a GenFsCon<'a>) -> Result<Self> {
        let result = vec![AtomicStatement {
            cmd: crate::sks_uapi::SKS_SEPOLICY_CMD_GENFSCON,
            subcmd: 0,
...
const fn cmd_expected_argc(cmd: u32) -> Option<usize> {
    match cmd {
        crate::sks_uapi::SKS_SEPOLICY_CMD_NORMAL_PERM
        | crate::sks_uapi::SKS_SEPOLICY_CMD_TYPE_CHANGE => Some(4),
        crate::sks_uapi::SKS_SEPOLICY_CMD_XPERM
        | crate::sks_uapi::SKS_SEPOLICY_CMD_TYPE_TRANSITION => Some(5),
        crate::sks_uapi::SKS_SEPOLICY_CMD_TYPE_STATE | crate::sks_uapi::SKS_SEPOLICY_CMD_ATTR => {
            Some(1)
        }
        crate::sks_uapi::SKS_SEPOLICY_CMD_TYPE | crate::sks_uapi::SKS_SEPOLICY_CMD_TYPE_ATTR => {
            Some(2)
        }
        crate::sks_uapi::SKS_SEPOLICY_CMD_GENFSCON => Some(3),
        _ => None,
    }
}
...
fn apply_rules_batch<'a>(statements: &'a [PolicyStatement<'a>], strict: bool) -> Result<()> {
    ...
    match crate::skcalls::set_sepolicy(payload.as_ptr(), payload.len() as u64) {
        Ok(applied_count) => {
