...
use crate::{defs, skcalls, module_config, utils};

const SKS_EVENT_QUEUE_TYPE_DROPPED: u16 = u16::MAX;
const SKS_EVENT_RECORD_FLAG_INTERNAL: u16 = 1;
...
pub const SULOG_CONFIG_MODULE_ID: &str = "internal.sksud.sulogd";
...
fn format_record_line(header: EventRecordHeader, payload: &[u8]) -> Result<String> {
    if header.record_type == SKS_EVENT_QUEUE_TYPE_DROPPED {
        ensure!(
            header.flags & SKS_EVENT_RECORD_FLAG_INTERNAL != 0,
...
pub fn spawn_sulogd() -> Result<()> {
    if utils::create_daemon(true)? {
        let current_exe = std::env::current_exe().context("failed to resolve current sksud path")?;
