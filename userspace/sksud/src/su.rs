...
use crate::skcalls::get_wrapped_fd;
use rustix::{
    process::getuid,
...
pub fn root_shell() -> Result<()> {
    ...
    opts.optflag(
        "",
        "ksu-no-new-privs",
        "Prevent this process (and its children) from privilege re-escalation via SxKernelSU",
    );
    ...
    if matches.opt_present("v") {
        println!("{}:SxKernelSU", defs::VERSION_NAME);
        return Ok(());
    }
    ...
    let sksu_no_new_privs = matches.opt_present("ksu-no-new-privs");
    ...
    if sksu_no_new_privs {
        skcalls::set_sksu_no_new_privs().context("set SKS_NO_NEW_PRIVS")?;
    }
