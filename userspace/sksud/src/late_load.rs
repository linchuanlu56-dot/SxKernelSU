...
pub fn run(package_name: &String, kmi: Option<String>, allow_shell: bool) -> Result<()> {
    ...
    // 1. Check if SxKernelSU is already loaded
    if sksuinit::has_sxkernelsu() {
        info!("SxKernelSU already loaded, skip loading ko");
    } else {
        ...
        // 3. Get sxkernelsu.ko from embedded assets
        let ko_name = format!("{kmi}_sxkernelsu.ko");
        let ko_data = assets::get_asset_data(&ko_name)
            .with_context(|| format!("Failed to get {ko_name} from assets"))?;

        // 4. Load sxkernelsu.ko from memory with manual relocation
        info!("Loading sxkernelsu.ko for KMI {kmi}...");
        let params = if allow_shell {
            cstr!("allow_shell=1")
        } else {
            cstr!("")
        };
        sksuinit::load_module(&ko_data, params).context("Failed to load sxkernelsu.ko")?;
        info!("sxkernelsu.ko loaded successfully!");
    ...
    utils::install(None).context("Failed to install sksud")?;
    ...
    info!("Restarting SxKernelSU Manager {package_name}...");
    ...
    let _ = Command::new("am")
        .args([
            "start",
            "-n",
            &format!("{package_name}/com.sxkernelsu.ui.MainActivity"),
        ])
        .status();
