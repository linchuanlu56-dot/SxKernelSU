...
const KERNEL_PARAM_PATH: &str = "/sys/module/sxkernelsu";
...
fn set_kernel_param(appid: u32) -> Result<()> {
    ...
    let sksu_debug_manager_appid = kernel_param_path.join("sksu_debug_manager_appid");
    let before_appid = read_u32(&sksu_debug_manager_appid)?;
    std::fs::write(&sksu_debug_manager_appid, appid.to_string())?;
    let after_appid = read_u32(&sksu_debug_manager_appid)?;
...
pub fn set_manager(pkg: &str) -> Result<()> {
        ...
        "CONFIG_SKS_DEBUG is not enabled"
    );
...
pub fn insmod(module: &Path, params: &[String]) -> Result<()> {
    ...
    sksuinit::load_module(&module_data, &cparams)
...
pub fn mark_get(pid: i32) -> Result<()> {
    let result = skcalls::mark_get(pid)?;
    if pid == 0 {
