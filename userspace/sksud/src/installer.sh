...
check_sepolicy() {
    /data/adb/sksd sepolicy check "$1"
    return $?
}
...
check_managed_features() {
  ...
  echo "$MANAGED_FEATURES" | tr ',' '\n' | while read -r feature; do
    ...
    # Check feature status using sksud
    local status=$(/data/adb/sksd feature check "$feature" 2>/dev/null)
...
install_module() {
  ...
  if is_legacy_script; then
    ...
    print_title "Powered by SxKernelSU"
