...
mod android {
    use super::Result;
    pub(super) use crate::defs::{BACKUP_FILENAME, SKS_BACKUP_DIR, SKS_BACKUP_FILE_PREFIX};
    use android_bootimg::cpio::{Cpio, CpioEntry};
    ...
    pub(super) fn do_backup(cpio: &mut Cpio, image: &Path) -> Result<()> {
        ...
        let filename = format!("{SKS_BACKUP_FILE_PREFIX}{sha1}");

        println!("- Backup stock boot image");
        let target = format!("{SKS_BACKUP_DIR}{filename}");
    ...
    pub(super) fn clean_backup(sha1: &str) -> Result<()> {
        ...
        let backup_name = format!("{SKS_BACKUP_FILE_PREFIX}{sha1}");
        let dir = std::fs::read_dir(SKS_BACKUP_DIR)?;
        for entry in dir.flatten() {
            ...
            if let Some(name) = path.file_name() {
                ...
                if name != backup_name
                    && name.starts_with(SKS_BACKUP_FILE_PREFIX)
...
pub struct BootPatchArgs {
    ...
    /// Do not (re-)install sxkernelsu, only modify configs (allow_shell, etc.)
    #[arg(long, default_value = "false")]
    no_install: bool,
...
pub fn patch(args: BootPatchArgs) -> Result<()> {
    let inner = move || {
        ...
        let sxkernelsu_ko: Box<dyn AsRef<[u8]>> = if no_install {
            Box::new(Vec::<u8>::new())
        } else if let Some(kmod_path) = kmod {
            Box::new(map_file(&kmod_path)?)
        } else {
            println!("- KMI: {kmi}");
            let name = format!("{kmi}_sxkernelsu.ko");
            assets::get_asset(&name).with_context(|| format!("Failed to load {name}"))?
        };

        let sksu_init: Box<dyn AsRef<[u8]>> = if no_install {
            Box::new(Vec::<u8>::new())
        } else if let Some(init_path) = init {
            Box::new(map_file(&init_path)?)
        } else {
            assets::get_asset("sksuinit").context("Failed to load sksuinit")?
        };
        ...
        if !no_install {
            ...
            println!("- Adding SxKernelSU LKM");
            let is_sxkernelsu_patched = cpio.exists("sxkernelsu.ko");

            if !is_sxkernelsu_patched && cpio.exists("init") {
                cpio.mv("init", "init.real")?;
            }

            cpio.add("init", CpioEntry::regular(0o755, sksu_init))?;
            cpio.add("sxkernelsu.ko", CpioEntry::regular(0o755, sxkernelsu_ko))?;

            #[cfg(target_os = "android")]
            if !is_sxkernelsu_patched
                && flash
        ...
        let mut sksu_config: Vec<String> = cpio
            .entry_by_name("sksu_config")
        ...
        let mut apply_config = |name: &str, value: &str, add: bool| {
            let has_value = sksu_config.iter().any(|v| v == value);

            if add {
                println!("- Adding {name} config");
                if !has_value {
                    sksu_config.push(value.to_owned());
                }
            } else if has_value {
                println!("- Removing {name} config");
                sksu_config.retain(|v| v != value);
            }
        };
        ...
        if sksu_config.is_empty() {
            cpio.rm("sksu_config", false);
        } else {
            let data = sksu_config.join(" ").into_bytes();
            cpio.add("sksu_config", CpioEntry::regular(0o644, Box::new(data)))?;
        }
        ...
        if should_write_output {
            ...
            let name = out_name.unwrap_or_else(|| {
                ...
                format!("sxkernelsu_patched_{}.img", now.format("%Y%m%d_%H%M%S"))
            });
...
pub fn restore(args: BootRestoreArgs) -> Result<()> {
    ...
    let (mut cpio, vendor_ramdisk_idx) =
        ...
        cpio.exists("sxkernelsu.ko"),
        "boot image is not patched by SxKernelSU"
    );
    ...
    if let Some(backup_file) = cpio.entry_by_name(BACKUP_FILENAME) {
        ...
        let sha = sha.trim();
        let backup_path =
            PathBuf::from(SKS_BACKUP_DIR).join(format!("{SKS_BACKUP_FILE_PREFIX}{sha}"));
    ...
    if should_write_output {
        ...
        let name = out_name.unwrap_or_else(|| {
            let now = chrono::Utc::now();
            format!("sxkernelsu_restore_{}.img", now.format("%Y%m%d_%H%M%S"))
        });
        let output_image = output_dir.join(name);
...
fn rebuild_without_ksu(
    ...
    println!("- Removing SxKernelSU from boot image");
    cpio.rm("sxkernelsu.ko", false);
