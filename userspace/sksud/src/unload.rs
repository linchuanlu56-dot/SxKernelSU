...
/// Find PIDs of processes running in the SxKernelSU su domain (u:r:ksu:s0).
...
/// Find PIDs of processes holding sksu_driver or sksu_fdwrapper file descriptors.
/// Returns a list of PIDs excluding our own.
fn find_sksu_fd_holders() -> Vec<i32> {
    let my_pid = std::process::id() as i32;
    let mut pids = Vec::new();
    ...
    for entry in entries.flatten() {
        ...
        for fd_entry in fds.flatten() {
            ...
            if let Ok(target) = fs::read_link(&link_path) {
                ...
                if target_str.contains("[sksu_driver]") || target_str.contains("[sksu_fdwrapper]") {
                    pids.push(pid);
                    break;
                }
            }
...
/// Close all sksu_driver and sksu_fdwrapper fds held by the current process.
fn close_sksu_fds() {
...
pub fn unload() -> Result<()> {
    info!("unload: starting SxKernelSU unload sequence");
    ...
    let fd_pids = find_sksu_fd_holders();
    if !fd_pids.is_empty() {
        info!(
    ...
    // 3. Close all our own sksu_driver and sksu_fdwrapper fds
    info!("unload: closing all ksu fds...");
    close_sksu_fds();

    // 4. delete_module("sxkernelsu")
    info!("unload: removing sxkernelsu module...");
    if let Err(e) = rustix::system::delete_module(c"sxkernelsu", 0) {
        warn!("unload: delete_module sxkernelsu failed: {e}");
    }
    ...
    // 6. Exit
    info!("unload: done, exiting sksud");
    std::process::exit(0);
}
