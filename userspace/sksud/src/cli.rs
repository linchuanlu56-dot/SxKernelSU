...
use crate::{
    apk_sign, assets, debug, defs, init_event, sks_uapi, skcalls, module, module_config, sulog,
    utils,
};

/// SxKernelSU userspace cli
#[derive(Parser, Debug)]
...
enum Commands {
    ...
    /// Run sulog reader daemon. Not for user. Use `sksud debug sulogd` to launch daemon.
    #[command(hide = true)]
    Sulogd,
    ...
    LateLoad {
        ...
        #[arg(long, default_value_t = String::from("com.sxkernelsu"))]
        package_name: String,
    },
    ...
    /// Install SxKernelSU userspace component to system
    Install {
        #[arg(long, default_value = None)]
        libadbroot: Option<PathBuf>,
    },

    /// Unload SxKernelSU kernel module (LKM Only)
    Unload,

    /// Uninstall SxKernelSU modules and itself(LKM Only)
    Uninstall {
        #[arg(long, default_value_t = String::from("com.sxkernelsu"))]
        package_name: String,
    },
    ...
    /// Restore boot or init_boot images patched by SxKernelSU
...
#[derive(clap::Subcommand, Debug)]
enum Debug {
    /// Set the manager app, kernel CONFIG_SKS_DEBUG should be enabled.
    SetManager {
        /// manager package name
        #[arg(default_value_t = String::from("com.sxkernelsu"))]
        apk: String,
    },
...
pub fn run() -> Result<()> {
    ...
    let result = match cli.command {
            ...
            match command {
                ...
                Module::Config { internal, command } => {
                    let module_id = match internal {
                        ...
                        None => std::env::var("SKS_MODULE").map_err(|_| {
                            anyhow::anyhow!(
        ...
        Commands::Services => {
            if skcalls::get_version() <= 0 {
                info!("SxKernelSU not available, exiting services");
                std::process::exit(0);
            }
        ...
        Commands::Debug { command } => match command {
            ...
            Debug::Version => {
                println!("Kernel Version: {}", skcalls::get_version());
                Ok(())
            }
            ...
            Debug::Info => {
                ...
                println!(
                    "lkm: {}",
                    (info.flags & sks_uapi::SKS_GET_INFO_FLAG_LKM) != 0
                );
                println!(
                    "late_load: {}",
                    (info.flags & sks_uapi::SKS_GET_INFO_FLAG_LATE_LOAD) != 0
                );
                println!(
                    "pr_build: {}",
                    (info.flags & sks_uapi::SKS_GET_INFO_FLAG_PR_BUILD) != 0
                );
                Ok(())
            }
        },
        ...
        Commands::Kernel { command } => match command {
            Kernel::NukeExt4Sysfs { mnt } => skcalls::nuke_ext4_sysfs(&mnt),
            Kernel::Umount { command } => match command {
                UmountOp::Add { mnt, flags } => skcalls::umount_list_add(&mnt, flags),
                UmountOp::Del { mnt } => skcalls::umount_list_del(&mnt),
                UmountOp::Wipe => skcalls::umount_list_wipe().map_err(Into::into),
            },
            Kernel::NotifyModuleMounted => {
                skcalls::report_module_mounted();
                Ok(())
            }
        },
