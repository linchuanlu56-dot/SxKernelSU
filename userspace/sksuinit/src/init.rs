...
pub fn init() -> Result<()> {
    ...
    if sksuinit::has_sxkernelsu() {
        log::info!("SxKernelSU may be already loaded in kernel, skip!");
    } else {
        log::info!("Loading sxkernelsu.ko..");
        if let Err(e) = load_module_from_path("/sxkernelsu.ko") {
            log::error!("Cannot load sxkernelsu.ko: {:?}", e);
        }
    }
...
fn load_module_from_path(path: &str) -> Result<()> {
    ...
    let params = std::fs::read("/sksu_config").unwrap_or_default();
    let params = unsafe { CString::from_vec_unchecked(params) };
    log::info!("load sxkernelsu with params {params:?}");
    sksuinit::load_module(&buffer, &params)
}
