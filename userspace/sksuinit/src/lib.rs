...
fn has_sxkernelsu_legacy() -> bool {
    ...
    log::info!("SxKernelSU version: {}", version);

    version != 0
}

fn has_sxkernelsu_v2() -> bool {
    use syscalls::{Sysno, syscall};
    const SKS_INSTALL_MAGIC1: u32 = 0xDEADBEEF;
    const SKS_INSTALL_MAGIC2: u32 = 0xCAFEBABE;
    const SKS_IOCTL_GET_INFO: u32 = 0x80104b02; // _IOR('K', 2, struct sksu_get_info_cmd)
    const SKS_IOCTL_GET_INFO_LEGACY: u32 = 0x80004b02; // _IOC(_IOC_READ, 'K', 2, 0)
    ...
    let version = if fd >= 0 {
        ...
        let version = unsafe {
            let ret = syscall!(Sysno::ioctl, fd, SKS_IOCTL_GET_INFO, &mut cmd as *mut _);
...
pub fn has_sxkernelsu() -> bool {
    has_sxkernelsu_v2() || has_sxkernelsu_legacy()
}
