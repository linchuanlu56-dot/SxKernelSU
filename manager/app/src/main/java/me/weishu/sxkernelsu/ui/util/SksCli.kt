package com.sxkernelsu.ui.util
...
private fun getKsuDaemonPath(): String {
    return ksuApp.applicationInfo.nativeLibraryDir + File.separator + "libsksud.so"
}
...
fun getRootShell(globalMnt: Boolean = false): Shell {
    return if (globalMnt) SksCli.GLOBAL_MNT_SHELL else {
        SksCli.SHELL
    }
}
...
fun flashModule(
    ...
    with(resolver.openInputStream(uri)) {
        ...
        Log.i("SxKernelSU", "install module $uri result: $result")
...
fun runModuleAction(
    ...
    Log.i("SxKernelSU", "Module runAction result: $result")
...
fun installBoot(
    ...
    when (lkm) {
        is LkmSelection.LkmUri -> {
            lkmFile = with(resolver.openInputStream(lkm.uri)) {
                val file = File(ksuApp.cacheDir, "sxkernelsu-tmp-lkm.ko")
                file.outputStream().use { output ->
                    this?.copyTo(output)
                }
    ...
    val result = flashWithIO("${getKsuDaemonPath()} $cmd", onStdout, onStderr)
    Log.i("SxKernelSU", "install boot result: ${result.isSuccess}")
    ...
    if (showReboot) { // because we decide do not update sksud when startActivity
        install() // install sksud here
    }
