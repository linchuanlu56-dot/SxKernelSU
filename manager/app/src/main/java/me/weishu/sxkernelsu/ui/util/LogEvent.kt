...
fun getBugreportFile(context: Context): File {
    ...
    val ksuFileSize = File(bugreportDir, "sksu_size.txt")
    ...
    shell.newJob().add("tar -czf ${bootlogFile.absolutePath} -C /data/adb/sks/log .").exec()
    ...
    shell.newJob().add("du -sh /data/adb/sks/* > ${ksuFileSize.absolutePath}").exec()
    shell.newJob().add("cp /data/system/packages.list ${appListFile.absolutePath}").exec()
    shell.newJob().add("getprop > ${propFile.absolutePath}").exec()
    shell.newJob().add("cp /data/adb/sks/.allowlist ${allowListFile.absolutePath}").exec()
    ...
    PrintWriter(FileWriter(buildInfo)).use { pw ->
        ...
        pw.println("SxKernelSU: $ksuKernel")
    ...
    val targetFile = File(context.cacheDir, "SxKernelSU_bugreport_${current}.tar.gz")
